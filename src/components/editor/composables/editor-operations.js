...
export class EditorOperations {
  ...
  createNode(type, data, notify) {
    this.model.value.applyEdits(
      [CreateNode.createDef({ ...data, type })],
      notify
    );
  }

  createNodeInCurrentViewWithOffset(
    type,
    viewport,
    positionOffset,
    data,
  ...
  createNodeInCurrentViewWithCascade(
    type,
    viewport,
    startOffset,
    cascadeOffset,
  ...
  newNodeInCurrentViewByMousePosition(
    ...
    if (containerBounds.contains(containerPoint)) {
      const layerPoint =
        canvasInstance.containerPointToLayerPoint(containerPoint);
  ...
  deleteModelObject(modelObject, notify) {
    ...
    switch (modelType) {
      ...
      case GraphModelType.Port:
        editOperations.push(
          DeleteConnectionsByPort.createDef(modelObject.nodeId, modelObject.id)
        );
        editOperations.push(
          DeletePort.createDef(modelObject.nodeId, modelObject.id)
        );
        break;
    }
  ...
  _deleteNodeWithConnnections(nodeId) {
    ...
    editOperations.push.apply(
      editOperations,
      connections.map(c => DeleteConnection.createDef(c.definition))
    );
